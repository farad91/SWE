...
io::NetCdfWriter::NetCdfWriter(char* CPfile)
{
    int err_val;
        ...
        // open .nc file
        nc_open(CPfile, NC_NOWRITE, &dataFile);
        ...
        nc_inq_dimid(dataFile, "x", &dim_x);
        nc_inq_dimid(dataFile, "y", &dim_y);
        
        size_t hnX;
        size_t hnY;
        nc_inq_dimlen(dataFile, dim_x, &hnX);
        nc_inq_dimlen(dataFile, dim_y, &hnY);
        io::Writer(hnX,HnY);
};    
/**
  ...
  flush(i_flush)
{
  ...
  //check if the netCDF-file creation constructor succeeded.
	...
	nc_def_dim(dataFile, "b", 4, &l_bDim);
